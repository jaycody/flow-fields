class FlowField {
  ...
  void squareRotate() {
    speed = map(mouseX,0,width, .0001, 1);
    for (int i = 0; i < cols; i++) {
      for (int j=0; j<rows; j++) {
        ...
        float radius = map(theta, -PI, PI, 20, 90);
        float strokeTheta = map(theta, -PI, PI, 20, 40);
        //println(theta);
        fill(strokeTheta, strokeTheta);
        stroke(255-strokeTheta, 255-strokeTheta);
        pushMatrix(); 
