class FlowField {
  ...
  void squareRotate() {
    speed = map(mouseX, 0, width, .001, 1);
    for (int i = 0; i < cols; i++) {
      for (int j=0; j<rows; j++) {
       ...
       // fill(strokeTheta, strokeTheta/theta);
        ...
        fill(strokeTheta*theta/2, strokeTheta*.5);
        stroke(strokeTheta*radius, 255*(strokeTheta/255)*1.3);
        pushMatrix(); 
        translate(squareLoc.x, squareLoc.y);
        //rotate(theta);
        float dir = (map(theta, -PI, PI, 1, -1));
        //if (dir<0) rot*=-1;
        rotate(squareRot.heading2D()*rot);
        //rect(0,0, radius, radius);
        rect(0, 0, radius*cos(theta)*3, radius*sin(theta)*3);
        //rect(0, 0, (strokeTheta*cos(theta))*dir, (strokeTheta*sin(theta))*dir);
        //rect(0, 0, (radius*cos(theta))*theta*1.2, (radius*sin(theta))*theta*1.2);
        popMatrix();
